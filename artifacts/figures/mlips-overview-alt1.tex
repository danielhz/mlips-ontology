% ALT 1: New bottom row.
% All four new leaf classes (DatasetProvenance, CoveredProperty,
% MetricType, MetricProperty) sit on a single new row at y=-0.8.
% trainedWith shortcut: long arc on the left of the central column.
\begin{tikzpicture}[
    x=1cm, y=1cm,
  ...
  % NEW LEAF NODES — single row at y=-0.8
  \node[trainnode]  (DatasetProvenance) at (3,    -0.8) {DatasetProvenance};
  \node[trainnode]  (CoveredProperty)   at (7,    -0.8) {CoveredProperty};
  \node[benchnode]  (MetricType)        at (12.5, -0.8) {MetricType};
  \node[benchnode, anchor=east]  (MetricProperty)    at (16.5, -0.8) {MetricProperty};
  ...
  \draw[edge] (TrainingDataset) -- node[edgelabel] {coversProperty} (CoveredProperty);
  \draw[edge] (AccuracyMetric) -- node[edgelabel] {metricType} (MetricType);
  \draw[edge] (AccuracyMetric) -- node[edgelabel] {metricProperty} (MetricProperty);
  ...
  % trainedWith shortcut: dotted arc on left of central column.
