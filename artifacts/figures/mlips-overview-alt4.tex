% ALT 4: Pairs side-by-side below each parent.
% DatasetProvenance + CoveredProperty: side-by-side directly below
% TrainingDataset.  MetricType + MetricProperty: side-by-side directly
% below AccuracyMetric (each pair shifted to fit).
% trainedWith shortcut: short dotted arc on the right of the central
% column (close to the column).
\begin{tikzpicture}[
    x=1cm, y=1cm,
  ...
  \draw[edge] (AccuracyMetric.south) -- node[edgelabel] {metricType} (MetricType);
  \draw[edge] (AccuracyMetric.south) -- node[edgelabel] {metricProperty} (MetricProperty);
  ...
  % trainedWith shortcut: tighter arc on the right of the central column
  \draw[shortcut, bend right=45, looseness=1.0]
