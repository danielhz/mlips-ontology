% ALT 3: Stacks below each parent.
% DatasetProvenance below TrainingDataset, then CoveredProperty below
% DatasetProvenance — vertical stack.  MetricType + MetricProperty
% similarly stacked below AccuracyMetric.
% trainedWith: dotted arc on the LEFT of the central column.
\begin{tikzpicture}[
    x=1cm, y=1cm,
  ...
  % NEW LEAF NODES — stacks below each parent
  \node[trainnode]  (DatasetProvenance) at (5,    -0.8) {DatasetProvenance};
  \node[trainnode]  (CoveredProperty)   at (5,    -2.0) {CoveredProperty};
  \node[benchnode, anchor=east] (MetricType)        at (16.5, -0.8) {MetricType};
  \node[benchnode, anchor=east] (MetricProperty)    at (16.5, -2.0) {MetricProperty};
  ...
  % NEW EDGES (vertical from parents)
  \draw[edge] (TrainingDataset) -- node[edgelabel] {datasetProvenance} (DatasetProvenance);
  \draw[edge] (TrainingDataset.south west) to[out=270, in=180]
    node[edgelabel, pos=0.6] {coversProperty} (CoveredProperty.west);
  \draw[edge] (AccuracyMetric) -- node[edgelabel] {metricType} (MetricType);
  \draw[edge] (AccuracyMetric.south east) to[out=270, in=0]
    node[edgelabel, pos=0.6] {metricProperty} (MetricProperty.east);
  ...
  % trainedWith shortcut: dotted arc on left.
  \draw[shortcut, bend left=70, looseness=1.5]
    (TrainedModel.west) to node[edgelabel, pos=0.5] {trainedWith} (MLIPMethod.west);
\end{tikzpicture}
