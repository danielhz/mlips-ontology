% ALT 2: Right-column extension.
% DatasetProvenance + CoveredProperty: side-by-side below TrainingDataset.
% MetricType + MetricProperty: stacked vertically to the RIGHT of
%   AccuracyMetric (new column at x=17, west-anchored).
% trainedWith shortcut: dotted arc on the right of the central column.
\begin{tikzpicture}[
    x=1cm, y=1cm,
  ...
  % NEW LEAF NODES
  % Dataset leaves: side-by-side below TrainingDataset
  \node[trainnode] (DatasetProvenance) at (3.5, -0.8) {DatasetProvenance};
  \node[trainnode] (CoveredProperty)   at (6.5, -0.8) {CoveredProperty};
  % Metric leaves: vertical stack to the right (new column at x=17)
  \node[benchnode, anchor=west] (MetricType)     at (17, 1.05) {MetricType};
  \node[benchnode, anchor=west] (MetricProperty) at (17, -0.05) {MetricProperty};
  ...
  % NEW EDGES
  \draw[edge] (TrainingDataset.south) to[out=270, in=90]
    node[edgelabel] {datasetProvenance} (DatasetProvenance.north);
  \draw[edge] (TrainingDataset.south) to[out=270, in=90]
    node[edgelabel] {coversProperty} (CoveredProperty.north);
  \draw[edge] (AccuracyMetric.east) to node[edgelabel, pos=0.5] {metricType} (MetricType.west);
  \draw[edge] (AccuracyMetric.east) to node[edgelabel, pos=0.5] {metricProperty} (MetricProperty.west);
