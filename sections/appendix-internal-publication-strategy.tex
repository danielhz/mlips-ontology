...
\item Deep dive on the polymorphic-schema and sub-property
  generalisation patterns (already drafted in
  \S\ref{sec:future-reference-method} of the SWJ scaffold).
...
Different first-authorship is natural: Daniel leads the SWJ paper;
Pranav or Yuji leads the npj paper, with Daniel as co-corresponding.
This gives the materials side ownership of their version.
...
the ontology gets adopted in practice. Worth confirming with Blazej
during the Monday meeting.

\subsection{Open Questions for Co-Author Discussion}

\begin{itemize}
\item Does Blazej have a current preference between npj Comp Mat,
  Scientific Data, and Digital Discovery for the MS sequel?
\item Who leads the MS paper (Pranav, Yuji, or shared)?
\item Should the MS sequel survey existing MLIP papers
  systematically? If so, who curates the corpus?
\item Does the SWJ version need a co-submission of the SHACL shapes
  to a separate venue (e.g., a workshop), or are they journal-body
  material?
\item Do we need approval from the META-LEARN ERC project for
  multi-paper publication on the same resource?
\end{itemize}
