...
This appendix describes all 24 classes defined in the \MLIPsOntology{}.

\subsection{$\dlConcept{Hyperparameter}$}
\label{sec:term-Hyperparameter}

\textbf{Label:} Hyperparameter.
A hyperparameter of an MLIP algorithm, with its name, data type, range, and default value.
...
A software implementation of an MLIP algorithm in a specific library and version.
...
A machine learning interatomic potential algorithm family (e.g., MACE, ACE, HDNNP, NequIP, M3GNet, MTP).

\paragraph{Superclasses.} $\dlConcept{mlschema:Algorithm}$.

\paragraph{Outgoing properties.} $\dlRole{hasHyperparameter}$, $\dlRole{hasImplementation}$, $\dlRole{supportsSimulation}$, $\dlRole{hasDescriptor}$, $\dlRole{trainingComplexity}$, $\dlRole{inferenceComplexity}$, $\dlRole{supportsGPU}$, $\dlRole{supportsParallelization}$.
...
\paragraph{Related axioms.} \eqref{ax:1}, \eqref{ax:2}.
...
The concrete artifact produced by training an algorithm on a specific dataset with specific hyperparameter settings---the learned parameters (weights) that can be used for prediction.
...
A concrete training execution: it executes an algorithm on a training dataset and produces a trained model.

\paragraph{Superclasses.} $\dlConcept{mlschema:Run}$, $\dlConcept{prov:Activity}$.

\paragraph{Outgoing properties.} $\dlRole{executes}$, $\dlRole{produces}$, $\dlRole{runsOn}$, $\dlRole{trainingDuration}$, $\dlRole{trainingHardware}$, $\dlRole{peakMemory}$, $\dlRole{gpuHours}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/TrainingRun.tex}{}{\textit{TODO: add example.}}

\paragraph{Related axioms.} \eqref{ax:4}, \eqref{ax:5}, \eqref{ax:6}.
...
A single evaluation result: an algorithm applied to a material system with specific hyperparameters, reporting accuracy metrics.
