...
This appendix describes all 35 classes defined in the \MLIPsOntology{}.
...
\paragraph{Incoming properties.} $\dlRole{implementedIn}$, $\dlRole{usedDFTCode}$, $\dlRole{usedReferenceCode}$.
...
\paragraph{Superclasses.} $\dlConcept{mlips:ReferenceCalculation}$, $\dlConcept{mdo:Calculation}$.
...
\paragraph{Superclasses.} $\dlConcept{mlips:ReferenceSettings}$.

\paragraph{Outgoing properties.} $\dlRole{dftBasisSet}$, $\dlRole{pseudopotentialType}$, $\dlRole{usedDFTCode}$, $\dlRole{xcFunctional}$, $\dlRole{energyCutoff}$, $\dlRole{kPointMesh}$.
...
\subsection{$\dlConcept{DftBasisSet}$}
\label{sec:term-DftBasisSet}

\textbf{Label:} DFT Basis Set.
The basis set used in a DFT calculation, when the code is all-electron or otherwise uses a localised basis (e.g., FHI-aims NAO families, LAPW, Gaussian-type orbitals such as cc-pVDZ or 6-31G). Range of \$\textbackslash\{\}dlRole\{dftBasisSet\}\$ on \$\textbackslash\{\}DFTSettingsC\$. Plane- wave codes specify the basis through \$\textbackslash\{\}dlRole\{energyCutoff\}\$ instead and leave this slot empty. Concrete basis sets are modelled as named individuals; an open extension point. Where the corpus uses a basis set not in the controlled vocabulary, a paper-local instance is minted and tagged with \$\textbackslash\{\}dlRole\{candidateForVocabulary\}\$ for later review.

\paragraph{Incoming properties.} $\dlRole{dftBasisSet}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/DftBasisSet.tex}{}{\textit{TODO: add example.}}
...
\subsection{$\dlConcept{PseudopotentialType}$}
\label{sec:term-PseudopotentialType}

\textbf{Label:} Pseudopotential Type.
A family of pseudopotentials used in a plane-wave DFT calculation (e.g., PAW, ultrasoft, norm-conserving). Range of \$\textbackslash\{\}dlRole\{pseudopotentialType\}\$ on \$\textbackslash\{\}DFTSettingsC\$. All-electron codes leave this slot empty. Concrete pseudopotential families are modelled as named individuals; an open extension point.

\paragraph{Incoming properties.} $\dlRole{pseudopotentialType}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/PseudopotentialType.tex}{}{\textit{TODO: add example.}}

\subsection{$\dlConcept{ReferenceCalculation}$}
\label{sec:term-ReferenceCalculation}

\textbf{Label:} Reference Calculation.
...
\paragraph{Outgoing properties.} $\dlRole{hasReferenceSettings}$.

\paragraph{Incoming properties.} $\dlRole{hasReferenceCalculation}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/ReferenceCalculation.tex}{}{\textit{TODO: add example.}}

\subsection{$\dlConcept{ReferenceSettings}$}
\label{sec:term-ReferenceSettings}

\textbf{Label:} Reference Settings.
...
\paragraph{Outgoing properties.} $\dlRole{usedReferenceCode}$.

\paragraph{Incoming properties.} $\dlRole{hasReferenceSettings}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/ReferenceSettings.tex}{}{\textit{TODO: add example.}}

\subsection{$\dlConcept{SamplingStrategy}$}
...
\paragraph{Incoming properties.} $\dlRole{samplingStrategy}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/SamplingStrategy.tex}{}{\textit{TODO: add example.}}
...
\paragraph{Outgoing properties.} $\dlRole{coversMaterial}$, $\dlRole{coversProperty}$, $\dlRole{datasetProvenance}$, $\dlRole{hasConfiguration}$, $\dlRole{hasDFTCalculation}$, $\dlRole{hasReferenceCalculation}$, $\dlRole{samplingStrategy}$, $\dlRole{numConfigurations}$.
...
\subsection{$\dlConcept{WaveFunctionCalculation}$}
\label{sec:term-WaveFunctionCalculation}

\textbf{Label:} Wave Function Calculation.
...
\paragraph{Superclasses.} $\dlConcept{mlips:ReferenceCalculation}$.

\paragraph{Outgoing properties.} $\dlRole{hasWaveFunctionSettings}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/WaveFunctionCalculation.tex}{}{\textit{TODO: add example.}}

\subsection{$\dlConcept{WaveFunctionSettings}$}
\label{sec:term-WaveFunctionSettings}

\textbf{Label:} Wave Function Settings.
...
\paragraph{Outgoing properties.} $\dlRole{wfMethod}$, $\dlRole{basisSet}$, $\dlRole{frozenCore}$.

\paragraph{Incoming properties.} $\dlRole{hasWaveFunctionSettings}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/WaveFunctionSettings.tex}{}{\textit{TODO: add example.}}

\subsection{$\dlConcept{WfMethod}$}
\label{sec:term-WfMethod}

\textbf{Label:} Wave Function Method.
A wave-function correlation method used to produce reference data (e.g., HF, MP2, CCSD, CCSD(T), DLPNO-CCSD(T), CASPT2, NEVPT2). Range of \$\textbackslash\{\}dlRole\{wfMethod\}\$ on \$\textbackslash\{\}WaveFunctionSettingsC\$. Concrete methods are modelled as named individuals; an open extension point.

\paragraph{Incoming properties.} $\dlRole{wfMethod}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/WfMethod.tex}{}{\textit{TODO: add example.}}

\subsection{$\dlConcept{XCFunctional}$}
\label{sec:term-XCFunctional}

\textbf{Label:} Exchange-Correlation Functional.
A density-functional approximation to the exchange-correlation energy used in a DFT calculation (e.g., LDA, PBE, PBE0, HSE06, SCAN, BLYP, omegaB97X). Range of \$\textbackslash\{\}dlRole\{xcFunctional\}\$ on \$\textbackslash\{\}DFTSettingsC\$. Concrete functionals are modelled as named individuals; an open extension point.

\paragraph{Incoming properties.} $\dlRole{xcFunctional}$.

\paragraph{Example.}
\InputIfFileExists{artifacts/examples/classes/XCFunctional.tex}{}{\textit{TODO: add example.}}
