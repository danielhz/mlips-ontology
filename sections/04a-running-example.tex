...
\begin{subfigure}[t]{0.48\textwidth}
  ...
  \fbox{\parbox{0.9\linewidth}{\centering\vspace{3cm}%
    2D cross-section showing $R_{\text{cut}}$\\and neighborhood $\mathbf{n}_i$%
    \vspace{3cm}}}
  \caption{The neighborhood $\mathbf{n}_i$ of atom~$i$: all atoms within
    $R_{\text{cut}}$ contribute to $V(\mathbf{n}_i)$. Atoms beyond
    $R_{\text{cut}}$ (grayed out) are excluded.}
  \label{fig:simulation-cell-cutoff}
\end{subfigure}
\caption{(a)~A Ti-Al simulation cell. (b)~The cutoff radius
  $R_{\text{cut}}$ defines the neighborhood $\mathbf{n}_i$, which
