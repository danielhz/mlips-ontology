...
\begin{figure}[t]
\centering
%% TODO: create simulation cell figure with two subfigures
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}%
  (a) 3D simulation cell with Ti (blue) and Al (orange) atoms.
  \hspace{1cm}
  (b) 2D cross-section showing the cutoff radius $R_{\text{cut}}$
  defining the neighborhood $\mathbf{n}_i$ of atom $i$.%
  \vspace{3cm}}}
\caption{(a)~A Ti-Al simulation cell with periodic boundary conditions.
  (b)~The neighborhood $\mathbf{n}_i$ of a central atom~$i$: all atoms
  within the cutoff radius $R_{\text{cut}}$ contribute to the local
  energy $V(\mathbf{n}_i)$. Atoms beyond $R_{\text{cut}}$ (grayed out)
  are excluded. The neighborhood includes the relative positions
  $\mathbf{r}_{ij}$ and the atomic types $z_j$ of the neighbors.}
...
\end{equation}
%
where $\mathbf{n}_i$ is the \emph{neighborhood} of atom~$i$, defined as
...
contribute to $V(\mathbf{n}_i)$---this \emph{locality assumption} is
what makes the potential computationally efficient.

The local energy $V(\mathbf{n}_i)$ is expressed as a linear combination
of basis functions:
%
